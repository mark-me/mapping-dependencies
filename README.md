...
* ```output/mapping_order_*.jsonl``` contains the ordering of the mapping executing, which can be adopted by the orchestrator.
...
* ```output/dag_run_report.html``` contains a graphical representation of the failure consequences, and
* ```output/dag_run_fallout.json``` contains a reports on the mappings/entities affected by the failure.
